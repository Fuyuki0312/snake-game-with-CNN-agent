...
DISABLE_EPSION_ON_n_GAMES = 1000
DISABLE_EPSION_ON_n_BEST_SCORE = 10
...
def train():
    ...
    while True:
        ...
        if game.is_game_over:
            ...
            # Disable epsilon when the agent get more experienced

            if (
                agent.n_games >= DISABLE_EPSION_ON_n_GAMES and
                agent.epsilon == 0.05 and
                agent.best_score >= DISABLE_EPSION_ON_n_BEST_SCORE
            ):
                agent.disable_epsilon()
                print("\n-------------- Agent's epsilon disabled --------------")
